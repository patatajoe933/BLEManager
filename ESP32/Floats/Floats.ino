  ...
  void onWrite(BLECharacteristic *pCharacteristic) override {
    ...
    if (dataLength > 0) {
      if (dataLength == 2) {  // 16-bit halfFloat
        ...
        uint32_t intValue = data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
        float floatValue;
        memcpy(&floatValue, &intValue, sizeof(float));
        Serial.println(floatValue);
      } else if (dataLength == 8) {  // 64-bit Double
        uint64_t intValue = static_cast<uint64_t>(data[0]) | (static_cast<uint64_t>(data[1]) << 8) | (static_cast<uint64_t>(data[2]) << 16) | (static_cast<uint64_t>(data[3]) << 24) | (static_cast<uint64_t>(data[4]) << 32) | (static_cast<uint64_t>(data[5]) << 40) | (static_cast<uint64_t>(data[6]) << 48) | (static_cast<uint64_t>(data[7]) << 56);
        double doubleValue;
